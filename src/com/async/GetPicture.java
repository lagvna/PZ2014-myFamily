...
/**
 * Klasa wywolujaca zdarzenie asynchroniczne odpowiadajace za pobranie zdjecia z serwera
 * @author lagvna
 *
 */
public class GetPicture extends AsyncTask<HttpResponse, Integer, String> {
	...
	public GetPicture(String dataArray[],
			PhotosManagementActivity callingActivity) {
	...
	protected void onPreExecute() {
		...
		callingActivity.mProgressDialog
	...
	protected String doInBackground(HttpResponse... arg0) {
		/*
		 * try { responseText = new HttpHandler(
		 * "http://malinowepi.no-ip.org/download_file.php", dataArray)
		 * .postGetPicture(); } catch (IOException e) { e.printStackTrace(); }
		 * return null;
		 */
		...
		try {
			...
			while (filesize > 0) {
				...
				totalDownload += len;
				callingActivity.mProgressDialog
						.setProgress((int) ((totalDownload / (float) totalSize) * 100));
	...
	protected void onPostExecute(String result) {
		...
		/*
		 * try { String loginResult = jp.getAddFamilyResult(); String tempArr[]
		 * = loginResult.split(":"); if (tempArr[0].equals("0")) {
		 * callingActivity.showToast("Cos poszlo zle: " + tempArr[1]); } else {
